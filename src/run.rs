...
/// Once a second, in units of the input timer.
const SETTINGS_CHECK_TICKS: u64 = 20;
/// Sent by a second launch to the copy already running.
const WM_SHOW_VIEWER: u32 = WM_APP + 2;

/// The guard: tray icon, focus hook, quick view. Returns when the user quits.
pub fn run_guard() {
    ...
    with_app(|app| {
        ...
        if first_run {
            ...
            tray::say(app.tray_window, s.first_run_title, s.first_run_body);
        }
    });
...
/// Normally one copy per machine. The tests set WSMF_INSTANCE so they can run a
/// measured copy alongside the one already in the tray.
fn instance_name() -> String {
    match std::env::var("WSMF_INSTANCE") {
        Ok(suffix) if !suffix.is_empty() => format!("{MUTEX_NAME}-{suffix}"),
        _ => MUTEX_NAME.to_owned(),
    }
}
    ...
    match message {
        ...
        WM_TIMER => {
            with_app(|app| {
                ...
                // The panel writes the settings file; notice it without hammering
                // the disk on every input poll.
                if app.ticks % SETTINGS_CHECK_TICKS == 0 && app.reload_config_if_changed() {
                    tray::refresh_tooltip(app);
                    viewer::refresh(app);
                }
            });
